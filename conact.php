...
if($_POST)
{
    ...
    if(!isset($_POST["userName"]) || !isset($_POST["userEmail"])  || !isset($_POST["subject"]) || !isset($_POST["userPhone"]) || !isset($_POST["userMessage"]))
    {
    ...
    if(strlen($user_Message)<5) //check emtpy message
    {
	 ...
	 if($cap != $verify) // If length is less than 4 it will throw an HTTP error.
    {
        $output = json_encode(array('type'=>'error', 'text' => 'Invalid Answers'));
        die($output);
    }
    ...
    */
    ...
    if(!$sentMail)
    {
        ...
        $output = json_encode(array('type'=>'message', 'text' => 'Hi <b>'.$user_Name .' </b> ' . ONSUCCESS));
        die($output);
    }
}
?>
